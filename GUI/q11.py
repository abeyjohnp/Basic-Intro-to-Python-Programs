#input two strings, concatenate when a button is pressed
from breezypythongui import EasyFrame
class buttondemo(EasyFrame):
    def __init__(self):
        EasyFrame.__init__(self)
        self.setSize(width=500, height=200)
        self.label=self.addLabel(text="Enter the first string : ",row=0,column=0)
        self.inputFld1=self.addTextField(text="",row=0,column=1)
        self.addLabel(text="Enter the second string : ",row=1,column=0)
